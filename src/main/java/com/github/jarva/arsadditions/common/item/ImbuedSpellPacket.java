...
public class ImbuedSpellParchment extends SpellParchment {
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack stack, int remainingUseDuration) {
        if (!(livingEntity.getCooldowns().isOnCooldown(stack.getItem()))) return;
        ...
        int cost = getSpellCaster(stack).getSpell().getCost();
        int seconds = -Math.floorDiv(-cost, 0.75);
        float value = tag.getString("Level");
        ...
        player.getCooldowns().addCooldown(stack, Math.floorDiv(seconds,value));
    }
